...
func main() {
	...
	fmt.Fprintln(os.Stderr, "║  🚀 Senior Architect RAG - Zero-Dependency MCP Server      ║")
	fmt.Fprintln(os.Stderr, "╚════════════════════════════════════════════════════════════╝")
...
Contém livros sobre System Design, DDD, Go, Microsserviços, etc.
Use para responder perguntas técnicas baseadas na literatura fornecida.`),
		mcp.WithString("pergunta",
			mcp.Required(),
			mcp.Description("A questão técnica ou padrão que deseja pesquisar"),
		),
	)
